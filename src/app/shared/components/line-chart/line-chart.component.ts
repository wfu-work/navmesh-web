...
export class LineChartComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input() colors: string[] = ['#0b8c5e', '#1677ff', '#fa8c16', '#eb2f96', '#13c2c2', '#722ed1'];
  @Input() yAxisName = '';
  @Input() xAxisName = '';
    ...
    return {
      ...
      yAxis: {
        ...
        axisLabel: {
          ...
          fontSize: 12,
          formatter:
            this.yAxisLabelFormatter || ((value: number) => `${value}${this.unit ? ` ${this.unit}` : ''}`),
        },
        axisLine: {
          show: false,
        },
          ...
          item.area ?? this.showArea
            ? {
                color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                  {
                    offset: 0,
                    color: this.withAlpha(item.color || this.colors[index % this.colors.length], 0.26),
                  },
                  {
                    offset: 1,
                    color: this.withAlpha(item.color || this.colors[index % this.colors.length], 0.02),
                  },
                ]),
              }
            : undefined,
